...
class FavoriteListingPresenter
{
	...
	*/
	...
	{
		$categories = '';
		$terms = get_the_terms( $this->favorite, 'session_category' );
				if ($terms) {
						$categories .= '<div class="fav-cat"><div>All</div>';
				    foreach($terms as $term) {
							$categories .= '<div>'.html_entity_decode($term->name).'</div>';
				    } 
		$categories .= '</div>';
		}

		$types = '';
		$terms = get_the_terms( $this->favorite, 'session_type' );
				if ($terms) {
						$types .= '<div class="fav-type"><div>All</div>';
				    foreach($terms as $term) {
							$types .= '<div>'.html_entity_decode($term->name).'</div>';
				    } 
		$types .= '</div>';
		}

		$skills = '';
		$terms = get_the_terms( $this->favorite, 'session_skills' );
				if ($terms) {
						$skills .= '<div class="fav-skills"><div>All</div>';
				    foreach($terms as $term) {
							$skills .= '<div>'.html_entity_decode($term->name).'</div>';
				    } 
		$skills .= '</div>';
		}

		$this->html = str_replace('[categories]', $categories, $this->html);
		$this->html = str_replace('[skills]', $skills, $this->html);	
		$this->html = str_replace('[types]', $types, $this->html);	
